...
- A few front-door sections use real data: `FeaturedStays` (real top-rated listings — now with real Prev/Next carousel controls, and equal-height cards regardless of how long a title wraps; previously a bare scrollable row with a real height-inconsistency bug when a title wrapped to two lines), `TestimonialSection` (real reviews via `GET /api/reviews/featured`), `DestinationGallery` (real listing photos — links to that listing's matching `/destinations/[slug]` page when its location is one of the 25 curated destinations, falling back to the listing's own `/apartments/[id]` page otherwise).
...
- Clicking a tile goes to `/destinations/[slug]` (`generateStaticParams` pre-renders all 25) — a tabbed page (Apartments/Hotels/Restaurants/Activities/Reviews, URL-driven via `?tab=`) built entirely from **our own** real listings/hotels/restaurants/reviews filtered by that destination's name, plus a curated Activities tab (see below).
- Apartments/Hotels/Restaurants each have their own inline search box (`.tab-search-form`, placeholder "Search for `<Tab>` nearby here") that replaces the destination's name with whatever the visitor types, rather than combining the two — the backend's search endpoints only take one keyword. Neither Activities nor Reviews has a search box — Reviews always uses the destination's name, and Activities (see below) is a fixed curated list, not a backend query.
- **Activities tab**: shows the same curated destination → activity associations `/plan` uses (`data/destinationActivities.ts`), as real tiles linking to each activity's own `/activities/[slug]` page — "search nearby activities" in practice means browsing this destination's curated activity list, not a live geo query. No backend call for this tab; it's a static lookup, same as `/plan`.
...
- The Hero search input has no destination `<datalist>`/autocomplete (removed by request) — it's plain free text, parsed by `/plan` (see below) rather than constrained to a fixed suggestion list, since suggesting only exact destination names contradicted accepting open-ended intent like "surfing in Fiji."
...
- `ActivitiesSection` shows `data/activityHighlights.ts` — a static, curated set of 14 activity types (Hiking, Surfing, Rock Climbing, Skiing, Mountaineering, History & Culture spots, etc. — Skiing and Mountaineering were added later, each tagged with a real, verified photo of that specific activity), each tagged with one or more of five categories (Adventure, High Adrenaline, Water Sports, History & Culture, Other Activities). Unlike the Nat Geo destinations, this is TripNest's own illustrative editorial content, not sourced from a third party — no attribution needed. Photos were spot-checked against what they actually depict — a few (Hiking, Surfing, Cycling) originally used photos that didn't match the labeled activity at all (a paraglider, an ancient pyramid, a neon city street) and were replaced with verified, correctly-matching photos.
...
- Both this carousel's tiles and `DestinationsCarousel`'s tiles have a hover/focus zoom effect — the whole tile scales up (`transform: scale(1.06)`, `z-index` raised above siblings), not just the photo inside it (an earlier version scaled only the image so neighboring tiles wouldn't shift — changed by request to scale the tile itself instead). Both carousels now carry extra top padding so the scaled-up tile doesn't get clipped by the track's `overflow-x: auto`.

**Plan your trip (`/plan`)** — the Hero's own search ("Start planning") no longer does a plain keyword search. It submits free text to `/plan?q=...`, which tries to understand *both* a destination and an activity in the same query (`lib/parseTripQuery.ts`) and shows results grouped by destination, using a new curated mapping (`data/destinationActivities.ts`) of which of the 14 activities are plausible at each of the 25 Nat Geo destinations.
...
- Each group has an Apartment/Hotel `<select>` + Find button (`<form method="GET">`, no client JS) that hands off to `/destinations/[slug]?tab=apartments` or `?tab=hotels` — reusing that page's existing real search rather than building a second one. The select is a real native `<select>` (keyboard/screen-reader behavior for free, works with zero JS) with custom CSS replacing the browser's default appearance (`appearance: none` + a `lucide-react` `ChevronDown` icon) rather than a JS-driven custom listbox. No pre-filled keyword beyond that page's own default (the destination's name); deeper handoff (e.g. carrying the activity into the tab's own search box) was left for later by request.
...
- Right side, in order: theme toggle icon (plain, no circle/pill background), then logged out shows `Login` + `Sign up`; logged in shows a "Hello `<name>`" dropdown trigger.
- Logged-in state is a real dropdown (`account-menu`/`account-menu-trigger`/`account-menu-dropdown` in `Navbar.tsx`) — the first client-side interactive menu in the app (click to open, closes on outside click, closes on Escape, proper `aria-haspopup`/`aria-expanded`/`role="menu"`/`role="menuitem"`). The trigger text uses `User.name` (real backend data — "Hello User 1" for the seeded demo account), which the navbar never actually displayed before; the previous version showed the email as plain text instead. The dropdown itself only has two items — **Settings** and **Logout**, both real. It was asked to also include Account/Chart/Wishlist/Bookings/Saved Plans/Estimates/Favorites, but none of those are real features anywhere in this app (no booking system, no wishlist/saved-plans concept at all — see Feature Gaps) — adding them would mean dead links or blank pages, which the destinations/activities pages' honest-empty-state pattern elsewhere in this app deliberately avoids. The menu mechanism is built and easy to extend; it's just only populated with what's real today.
...
- Fixed (not sticky — sticky still occupies flow space), backdrop-blurred, with a real translucent dark tint (`--color-navbar-bg`) rather than the fully transparent background it started with — pure blur-for-legibility didn't hold up in practice: the navbar sits over the Hero photo at the top of every page, and in light theme its near-black `--color-text` was barely readable against that photo. `--color-navbar-bg`/`--color-navbar-text` are deliberately theme-independent (same dark tint + light text in both themes), since the navbar always needs to read over the same dark hero regardless of site theme; `.nav-link` (Login), the theme toggle icon, the account menu trigger, and `BrandMark`'s second word all use `--color-navbar-text` specifically inside `.navbar` rather than the normal `--color-text`.
...
**Trip illustration banner**: `TripIllustrationBanner` renders a decorative image (`/public/images/trip-illustration-banner.jpg`) full-bleed, site-wide, directly above the footer (mounted in `AppShell`, not `page.tsx`) — the illustration from the original `ExperienceBanner`, brought back after a cleaned version of the same asset (text and button removed from the image itself) was provided. The provided export had a stray 1px border line baked into its top edge plus a large empty area above the illustration figures — together these rendered as a visible empty bordered gap right above the footer, so the image was re-cropped down to just the airplane + illustration strip. Real heading text ("The Value For Experience" / "Relax… You're with us! We make it simple.") is now overlaid on top of the image (not baked into it) to fill what was still a visually empty middle — same message as `SearchResultsIntro`, styled with hardcoded colors rather than theme variables since this image has a fixed white/light background regardless of site theme, and sized with `clamp()` rather than a fixed font-size since the image is full-bleed and its rendered height shrinks a lot on narrow viewports. No overlay link — nothing in the image is clickable, so only the `<Image>` itself is `alt=""` (decorative); the text is real, accessible content, so the section itself is not `aria-hidden`.
...
**Frontend** (`frontend/vitest.config.ts`): Vitest + React Testing Library component tests, co-located with components (`StarRating`, `StarRatingInput`, `SearchFilters`, `ApartmentCard`, `DestinationsCarousel`, `ThemeIcon`, `BrandMark`, `FaqSection`, `TestimonialSection`, `DestinationGallery`, `SettingsForm`, `ActivitiesSection`, `ExploreCategories`, `RestaurantCard`, `TripIllustrationBanner`, `FeaturedStays`, `Navbar` — the account dropdown's open/close, outside-click, Escape, and that it only ever lists Settings/Logout, mocking `AuthContext`/`ThemeContext` directly the same way `SettingsForm.test.tsx` does), plus a pure-function suite for `lib/parseTripQuery.ts` (no rendering involved — it's just string matching against the two curated vocabularies, including the "must not fuzzy-match a wrong place" case from the README note above).
...
- **Destination autocomplete** — the Hero search input has no destination suggestions/autocomplete (there used to be a `<datalist>` of the 25 Nat Geo names; removed once the search became intent-based free text — see "Plan your trip" above). True autocomplete against the full listings/hotels/restaurants catalog would need a dedicated suggestions endpoint as the catalog grows beyond a browsable size, and still doesn't exist.
