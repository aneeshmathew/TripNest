...
- Both this carousel's tiles and `DestinationsCarousel`'s tiles have a hover/focus zoom effect — the photo inside scales up (`transform: scale(1.08)`) while the tile's own box stays a fixed size, so neighboring tiles never shift or overlap.
