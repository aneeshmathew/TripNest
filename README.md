...
- No active search/filter → full marketing front door: `Hero`, `DestinationsSection`, `ActivitiesSection`, `FeaturedStays`, `EasyToUseSection`, `PlanWithFriendsSection`, `RecommendationsSection`, `FaqSection`, `TestimonialSection`, `DestinationGallery`.
...
- `PlanWithFriendsSection` / `RecommendationsSection` are explicitly illustrative marketing copy for features that don't exist yet (group planning, a recommendation engine) — not fabricated data pretending to be real.
